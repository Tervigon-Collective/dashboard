...
|-------|------|-------------|---------|
| `chart_type` | `string` | Type of chart to render. Valid values: `"bar"`, `"line"`, `"area"`, `"pie"`, `"donut"`, `"scatter"`, `"bubble"` | `"bar"` |
...
| `value_format` | `string` | **Optional hint for value formatting**. Values: `"percentage"`, `"currency"`, `"number"`. If not provided, auto-detected from labels/keys/data. | `"percentage"` |
| `data_format` | `string` | **Optional hint for data structure**. Values: `"long"`, `"pivoted"`. If not provided, auto-detected from data structure. | `"pivoted"` |
...
The frontend now **automatically detects** data formats and value types, making it highly flexible:

### Data Format Detection

The frontend supports **two data formats** and auto-detects which one you're using:

1. **Long Format** (default): Each row has `x_axis` + `y_axis` keys
   ```json
   {
     "day": "2025-11-01",
     "bounce_rate": 26.62
...
2. **Pivoted Format**: Each row has `x_axis` + series names as keys
   ```json
   {
     "day": "2025-11-01",
     "Campaign A": 26.62,
...
- If `y_axis` keys exist in data → uses **long format**
- If `y_axis` keys don't exist but other keys do → uses **pivoted format**
- You can override with `data_format: "long"` or `data_format: "pivoted"`

### Value Format Detection

The frontend **automatically detects** value formats:

1. **Percentage**: Detected if:
   - `y_label` contains '%' or 'percent'
   - `y_axis` keys contain 'rate', 'percent', or '%'
   - Values are 0-1 range with rate-related labels

2. **Currency**: Detected if:
   - `y_label` contains '₹', 'rupee', 'revenue', 'sales', 'price', 'cost'
   - Values are large (>1000) with financial keywords

3. **Number**: Default fallback for plain numeric values

**Auto-detection logic:**
- Analyzes `y_label`, `y_axis` keys, and sample data values
- You can override with `value_format: "percentage"`, `"currency"`, or `"number"`
...
- **Optional**: Add `value_format` and `data_format` hints for explicit control

**✅ FLEXIBLE AUTO-DETECTION:**
- The frontend automatically detects data format (long vs pivoted)
- The frontend automatically detects value format (percentage, currency, number)
- Works with any data structure without requiring explicit hints
- Override with optional `value_format` and `data_format` fields when needed

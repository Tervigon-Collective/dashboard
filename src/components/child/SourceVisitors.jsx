...
const SourceVisitors = () => {
  ...
  useEffect(() => {
    ...
    setError(null);
    setData(null);
    axios.get(`${config.api.baseURL}/api/source_visitor/${timeframe}`)
      .then(res => {
        if (!cancelled) {
          setData(res.data);
        }
  ...
  const total = data ? [data.fb, data.insta, data.google, data.organic].reduce((a, b) => a + (b || 0), 0) : 0;

  // Find the max value for dynamic bar heights
  const maxValue = data ? Math.max(data.fb || 0, data.insta || 0, data.google || 0, data.organic || 0) : 1;
    ...
    <div className='col-xxl-5'>
      <div className='card h-100'>
        <div className='card-header border-bottom-0 pb-0 d-flex align-items-center flex-wrap gap-2 justify-content-between'>
          ...
          >
            {allowed.map(opt => (
              <option key={opt} value={opt}>{labelMap[opt]}</option>
            ))}
          </select>
        </div>
          ...
          <div className='position-relative h-100 min-h-320-px'>
            ...
            <div className='row g-3 h-100'>
              {sortedBars.map((bar, idx) => (
                <div key={bar.key} className='col-12 col-md-3 d-flex flex-column justify-content-end'>
                  <div
                    className={`d-flex flex-column align-items-center p-24 pt-16 rounded-top-4 ${bar.bg}`}
                    style={{ minHeight: `${data && maxValue ? (bar.value / maxValue) * 100 : 50}%` }}
                  >
                    <span className={`w-40-px h-40-px d-flex flex-shrink-0 justify-content-center align-items-center ${bar.circle} rounded-circle mb-12`}>
                      <img
                        src={bar.icon}
                        alt={bar.label}
                      />
                    ...
                    <h6 className='mb-0'>{loading ? '--' : error ? '--' : (bar.value ?? '--')}</h6>
                  </div>
                </div>
              ))}
            </div>

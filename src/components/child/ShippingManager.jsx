...
const ShippingManager = ({ order, onShippingUpdate }) => {
  ...
  const fetchCouriers = async () => {
    try {
      const response = await fetch(`${config.api.baseURL}/api/shipping/couriers`);
  ...
  const trackShipment = async (awbNumber, courier = 'bluedart') => {
    ...
    try {
      const response = await fetch(`${config.api.baseURL}/api/shipping/track/${awbNumber}?courier=${courier}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const validateAddress = async () => {
    ...
    try {
      const response = await fetch(`${config.api.baseURL}/api/shipping/validate-address`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        ...
        <div className="card-body">
          ...
          <div className="row mb-3">
            <div className="col-md-6">
              ...
              <p className="mb-1"><strong>Customer:</strong> {order.shipping_name}</p>
              <p className="mb-1"><strong>Phone:</strong> {order.phone}</p>
              <p className="mb-1"><strong>Pincode:</strong> {order.shipping_zip}</p>
            </div>
            <div className="col-md-6">
              <h6>Shipping Address</h6>
              <p className="mb-1">{order.shipping_address1}</p>
              {order.shipping_address2 && <p className="mb-1">{order.shipping_address2}</p>}
              <p className="mb-1">{order.shipping_city}, {order.shipping_province_name}</p>
            </div>
          </div>
              ...
              <div className="col-12">
                ...
                <div className="card bg-light">
                  <div className="card-body">
                    <div className="row">
                      <div className="col-md-6">
                        <p><strong>Tracking Number:</strong> {trackingData.awb_number || trackingData.tracking_number}</p>
                        <p><strong>Courier:</strong> {trackingData.courier}</p>
                        <p><strong>Status:</strong> {trackingData.status || 'Generated'}</p>
                      </div>
                      <div className="col-md-6">
                        <a 
                  ...
                  <div className="card-body">
                    {order.awb_number && (
                      <div className="mb-2">
                        ...
                        <button 
                          ...
                          onClick={() => trackShipment(order.awb_number, 'bluedart')}
                        >
                          Track
                        </button>
                      </div>
                    ...
                    {order.ewaybill_number && (
                      <div>
                        ...
                        <button 
                          className="btn btn-sm btn-outline-primary ms-2"
                          onClick={() => trackShipment(order.ewaybill_number, 'dhl')}
                        >
                          Track
                        </button>
                      </div>

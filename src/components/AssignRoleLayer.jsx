...
const AssignRoleLayer = () => {
  ...
  const handleRoleAssignment = async (userId, newRole) => {
    if (!hasRole('super_admin') && !hasRole('admin')) {
      setMessage({ type: 'error', text: 'You do not have permission to assign roles' });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ type: 'success', text: `Role updated to ${getRoleDisplayName(newRole)}` });
  ...
  return (
    <div className='card h-100 p-0 radius-12'>
      ...
      {/* Message Display */}
      {message.text && (
        <div className={`alert alert-${message.type === 'success' ? 'success' : 'danger'} m-3`}>
          <Icon 
      ...
      <div className='card-body p-24'>
            ...
            <tbody>
                ...
                <td>
                  <div className='d-flex align-items-center'>
                    <img
                          src={userData.photoURL || '/assets/images/avatar/default-avatar.png'}
                      alt=''
        ...
        {!loading && filteredUsers.length === 0 && (
          <div className="text-center py-4">
            ...
            <p className="text-muted mt-2">No users found matching your criteria.</p>
          </div>
        )}

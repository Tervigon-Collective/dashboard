...
const ShippingDashboard = () => {
  ...
  useEffect(() => {
    ...
    const selectedCurrentPage =
      currentPageOrderNames.length > 0 &&
  ...
  const generateWaybillWithData = async (order) => {
    try {
      ...
      if (!validation.isValid) {
        alert(
          `Cannot generate waybill. Please fix the following issues:\n\n${validation.errors.join(
            "\n"
          )}`
        );
        return;
      ...
      const response = await fetch(
        `${config.api.baseURL}/api/shipping/generate-waybill`,
        {
          method: "POST",
          headers: {
      ...
      if (data.success) {
        alert(
          `Waybill generated successfully for ${
            order.order_name
  ...
  const bulkGenerateWaybills = async () => {
    ...
    try {
      const selectedOrderData = orders.filter(
        (order) =>
      ...
      if (selectedOrderData.length === 0) {
        ...
        const unavailableOrders = orders.filter(
          (order) =>
            selectedOrders.has(order.order_name) &&
            !isOrderCancelled(order) &&
            order.bluedart_service_available === false
        ...
        if (unavailableOrders.length > 0) {
          const unavailableNames = unavailableOrders
            .map((order) => order.order_name)
            .join(", ");
          alert(
            `Cannot generate waybills for the following orders due to BlueDart service unavailability:\n\n${unavailableNames}\n\nPlease check pincode and payment method compatibility.`
          );
        } else {
      ...
      const bulkWaybillData = selectedOrderData
        .map((order) => {
          const waybillData = createWaybillData(order);
          if (!waybillData) {
            console.error(
              `Unable to create waybill data for ${order.order_name}`
            );
            return null;
          }
          return waybillData;
      ...
      if (bulkWaybillData.length === 0) {
        alert(
          "No valid waybill data could be created for the selected orders."
        );
        return;
      }
      ...
      const response = await fetch(
        `${config.api.baseURL}/api/shipping/generate-bulk-waybills`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (data.success) {
        alert(
          `Bulk waybill generation completed successfully for ${bulkWaybillData.length} orders!`
        );
        ...
        alert(
          `Failed to generate bulk waybills: ${data.error || "Unknown error"}`
        );
      }
    } catch (error) {
  ...
  const trackShipment = async (
    ...
    try {
      const response = await fetch(
        `${config.api.baseURL}/api/shipping/track/${trackingNumber}?courier=${courier}`
      );
  ...
  const fetchTrackingHistory = async (trackingNumber) => {
    try {
      const response = await fetch(
        `${config.api.baseURL}/api/shipping/tracking-history/${trackingNumber}`
      );
  ...
  const formatProductNames = (order) => {
    ...
    if (itemCount <= 3) {
      ...
      return order.line_items
        .map((item) => item.lineitem_sku || "Unknown")
        .join(", ");
    } else {
  ...
  const createWaybillData = (order) => {
    ...
    if (itemCount <= 3) {
      ...
      commodityDetails = order.line_items.map(
        (item) => item.lineitem_sku || "Unknown"
      );
    } else {
    ...
    return {
      ...
      services: {
        ...
        Product1:
          itemCount <= 3
            ? order.line_items?.[0]?.lineitem_sku || "N/A"
            : `Accessories${itemCount}x`,
        SKU1: order.line_items?.[0]?.lineitem_sku || "N/A",
        SubProduct1_1:
          itemCount <= 3 ? order.line_items?.[1]?.lineitem_sku || "" : "",
        SubProduct1_2:
          itemCount <= 3 ? order.line_items?.[2]?.lineitem_sku || "" : "",
        InvoiceNumber: order.order_name || "",
        orderAmount: order.total || 0,
        itemCount:
          order.line_items?.reduce(
            (total, item) => total + (item.lineitem_quantity || 1),
            0
          ) || 1,
  ...
  const isOrderCancelled = (order) => {
    return (
      order.payment_status === "voided" || order.payment_status === "cancelled"
    );
  };
  ...
  const getShippingStatusDisplay = (order) => {
    ...
    if (order.bluedart_service_available === false) {
      return {
        ...
        tooltip:
          "BlueDart service not available for this pincode and payment method",
      };
    }
  ...
  const downloadTHWaybillPDF = async (awbNumber, orderName) => {
    if (!awbNumber) {
      alert(
        "No AWB number available for this order. Please generate an AWB first."
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `${config.api.baseURL}/api/shipping/download-th-pdf/${awbNumber}`
      );
      ...
      if (data.success && data.data?.pdf_base64) {
        ...
        link.setAttribute(
          "download",
          `TH-Waybill-${awbNumber}-${orderName || "Order"}.pdf`
        );
        ...
        alert(
          `Failed to download TH waybill: ${data.error || "Unknown error"}`
        );
      }
    } catch (error) {
  ...
  const downloadWaybillPDF = async (awbNumber, orderName) => {
    ...
    try {
      const response = await fetch(
        `${config.api.baseURL}/api/shipping/download-pdf/${awbNumber}`
      );
      ...
      if (contentType && contentType.includes("application/pdf")) {
        ...
        link.setAttribute(
          "download",
          `Waybill-${awbNumber}-${orderName || "Order"}.pdf`
        );
        link.style.visibility = "hidden";
        document.body.appendChild(link);
        link.click();
        ...
        try {
          ...
          if (data.success && data.data?.pdf_base64) {
            ...
            alert(
              `Failed to download waybill: ${data.error || "Unknown error"}`
            );
          }
        } catch (jsonError) {
  ...
  const downloadShippingLabel = async (awbNumber, orderName) => {
    ...
    try {
      ...
      if (contentType && contentType.includes("application/pdf")) {
        ...
        link.setAttribute(
          "download",
          `Shipping-Label-${awbNumber}-${orderName || "Order"}.pdf`
        );
        link.style.visibility = "hidden";
        document.body.appendChild(link);
        link.click();
        ...
        try {
          ...
          if (data.success && data.data?.pdf_base64) {
            ...
            alert(
              `Shipping label downloaded successfully for AWB: ${awbNumber}`
            );
          } else {
            alert(
  ...
  const bulkDownloadWaybills = async () => {
    ...
    const ordersWithAWB = selectedOrderData.filter(
      (order) => order.awb_number && !isOrderCancelled(order)
    );
    ...
    console.log(
      "Orders with AWB details for PDFs:",
      ordersWithAWB.map((order) => ({
    ...
    if (ordersWithAWB.length === 0) {
      alert(
        "No selected orders have AWB numbers or orders are cancelled. Please generate waybills first."
      );
      return;
    }
    ...
    if (ordersWithAWB.length !== selectedOrderData.length) {
      ...
      const missingOrders = selectedOrderData
        ...
        .map(
          (order) =>
            `${order.order_name} (${
              !order.awb_number ? "No AWB" : "Cancelled"
            })`
        );

      alert(
        `${missingAWB} selected order(s) will be skipped:\n\n${missingOrders.join(
          "\n"
    ...
    try {
      ...
      const response = await fetch(
        `${config.api.baseURL}/api/shipping/download-bulk-pdf`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (contentType && contentType.includes("application/pdf")) {
        ...
        link.setAttribute(
          "download",
          `Bulk-Waybills-${new Date().toISOString().split("T")[0]}.pdf`
        );
        link.style.visibility = "hidden";
        document.body.appendChild(link);
        link.click();
        ...
        alert(
          `Bulk waybill PDFs downloaded successfully for ${ordersWithAWB.length} orders!`
        );
      } else {
        // JSON response with base64 data
        try {
          ...
          if (data.success && data.data?.pdf_base64) {
            ...
            alert(
              `Failed to download bulk waybills: ${
                data.error || "Unknown error"
              }`
            );
          }
  ...
  const bulkDownloadLabels = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${config.api.baseURL}/api/shipping/download-bulk-th-pdf`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (contentType && contentType.includes("application/pdf")) {
        ...
        link.setAttribute(
          "download",
          `Bulk-Shipping-Labels-${new Date().toISOString().split("T")[0]}.pdf`
        );
        link.style.visibility = "hidden";
        document.body.appendChild(link);
        link.click();
        ...
        alert(
          `Bulk shipping labels downloaded successfully for ${ordersWithAWB.length} orders!`
        );
      } else {
        // JSON response with base64 data
        try {
          ...
          if (data.success && data.data?.pdf_base64) {
            ...
            alert(
              `Failed to download bulk shipping labels: ${
                data.error || "Unknown error"
              }`
            );
          }
          ...
          {selectedOrders.size > 0 && (
            <>
              <button
                ...
                title={`Generate waybills with product data for ${
                  selectedOrders.size
                } selected order${selectedOrders.size > 1 ? "s" : ""}`}
              >
                {bulkLoading && bulkOperation === "waybill" ? (
                  ...
                  <>
                    ...
                    <span className="d-none d-sm-inline">
                      Bulk Waybills ({selectedOrders.size})
                    </span>
                  </>
                )}
              </button>
              <button
                ...
                disabled={bulkLoading}
                title={`Download Waybill PDFs for ${
                  selectedOrders.size
                } selected order${selectedOrders.size > 1 ? "s" : ""}`}
              >
              ...
              <button
                ...
                disabled={bulkLoading}
                title={`Download Shipping Labels for ${
                  selectedOrders.size
                } selected order${selectedOrders.size > 1 ? "s" : ""}`}
              >
                {bulkLoading && bulkOperation === "downloadLabels" ? (
                  ...
                  <>
                    ...
                    <span className="d-none d-sm-inline">
                      Bulk Labels ({selectedOrders.size})
                    </span>
                  </>
                )}
              </button>
                        ...
                        >
                          <td className="border-end px-4 py-3 align-middle">
                            <div className="form-check d-flex justify-content-center">
                              <input
                                ...
                                onChange={() =>
                                  handleOrderSelection(order.order_name)
                                }
                                disabled={isOrderCancelled(order)}
                                title={
                                  isOrderCancelled(order)
                                    ? "Cannot select cancelled orders"
                                    : "Select this order"
                                }
                              />
                            </div>
                          ...
                          <td className="border-end px-4 py-3 align-middle">
                            <div className="d-flex flex-column">
                              <Link
                                href="#"
                                className="text-primary fw-semibold text-decoration-none mb-1"
                              >
                                {order.order_name}
                              </Link>
                              {isOrderCancelled(order) && (
                                <div className="d-flex align-items-center gap-1 mb-1">
                                  ...
                                  />
                                  <span
                                    className="badge bg-danger-subtle text-danger border border-danger rounded-pill px-2 py-1"
                                    style={{
                                      fontSize: "9px",
                                      fontWeight: "500",
                                      letterSpacing: "0.3px",
                              ...
                              {!isOrderCancelled(order) &&
                                !order.awb_number && (
                                  <div className="d-flex align-items-center gap-1 mb-1">
                                    ...
                                    />
                                    <span
                                      className="badge bg-warning-subtle text-warning border border-warning rounded-pill px-2 py-1"
                                      style={{
                                        fontSize: "9px",
                                        fontWeight: "500",
                                        letterSpacing: "0.3px",
                              ...
                              {!isOrderCancelled(order) &&
                                order.bluedart_service_available === true &&
                                !order.awb_number && (
                                  <div className="d-flex align-items-center gap-1 mb-1">
                                    <Icon
                                    ...
                                    />
                                    <span
                                      className="badge bg-success-subtle text-success border border-success rounded-pill px-2 py-1"
                                      style={{
                                        fontSize: "9px",
                                        fontWeight: "500",
                                        letterSpacing: "0.3px",
                          ...
                          <td className="border-end px-4 py-3 text-center align-middle">
                            {shippingStatus.type === "awb" ? (
                              ...
                              <span
                                ...
                                title={
                                  shippingStatus.text === "Missing Data"
                                    ? `Missing: ${validateOrderForWaybill(
                                        order
                                      ).errors.join(", ")}`
                                    : shippingStatus.text
                                }
                            ...
                            <div className="d-flex gap-1 justify-content-center">
                              {hasOperation("shipping", "create") && (
                                <button
                                  ...
                                  title={
                                    isOrderCancelled(order)
                                      ? "Cannot generate waybill for cancelled orders"
                                      : order.bluedart_service_available ===
                                        false
                                      ? "BlueDart service not available for this pincode and payment method"
                  ...
                  <div>
                    ...
                    {trackingData.status === "Delivered" && (
                      <div className="alert alert-success">
                        ...
                        <strong>Package Delivered!</strong> The shipment has
